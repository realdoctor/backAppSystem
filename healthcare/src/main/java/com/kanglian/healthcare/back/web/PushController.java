...
public class PushController extends BaseController {
    ...
    public @ResponseBody ResultBody pushmsg(@CurrentUser User user, @RequestBody PushmsgQuery query) throws Exception {
        ...
        // 用户信息
        Map<String, Object> userMap = userBo.getUserInfo(mobilePhone);
        String userName = userMap.get(Constants.USER_NAME) + "";
        String imageUrl = userMap.get(Constants.IMAGE_URL) + "";
        if (StringUtil.isNotEmpty(imageUrl)) {
            imageUrl = Constants.getStaticUrl().concat(imageUrl);
        }
        ...
        pushModel.addParam(Constants.USER_NAME, userName);
        pushModel.addParam(Constants.IMAGE_URL, imageUrl);
